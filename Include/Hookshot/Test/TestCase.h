...
/// harness. Treat this macro as a function declaration; the test case is the function body. Inside
/// the function body, the variable `hookshot` is available as a pointer to a Hookshot interface
/// object.
#define HOOKSHOT_TEST_CASE_CONDITIONAL(name, cond)                                                 \
  ...
  bool HookshotTest::TestCase<kHookshotTestName__##name>::CanRun(void) const                       \
  {                                                                                                \
    return (cond);                                                                                 \
  }                                                                                                \
...
namespace HookshotTest
{
  ...
  public:
    ...
    /// Constructs a test case object with an associated test case name, and registers it with the
    /// harness.
    ...
    /// Implementations are generated when test cases are created using the #HOOKSHOT_TEST_CASE
    /// macro.
  ...
  /// Each test case created by #HOOKSHOT_TEST_CASE instantiates an object of this type with a
