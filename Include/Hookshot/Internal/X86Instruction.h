...
namespace Hookshot
{
  ...
  public:
    ...
    static constexpr uint8_t kJumpInstructionPreamble[] = {0xe9};
    ...
    static constexpr int kJumpInstructionLengthBytes =
        sizeof(kJumpInstructionPreamble) + sizeof(uint32_t);

    /// Value used to indicate an invalid memory displacement.
    static constexpr int64_t kInvalidMemoryDisplacement = INT64_MIN;

    /// Holds information on a position-dependent memory reference that might be present in an X86
    /// instruction.
    class PositionDependentMemoryReference
    {
    public:

      /// Value that specifies that an instruction does not contain a position-dependent memory
      /// reference.
      static constexpr int kReferenceDoesNotExist = INT_MIN;

      /// Value that specifies that an instruction's position-dependent memory reference is a branch
      /// displacement.
      ...
      inline PositionDependentMemoryReference(xed_decoded_inst_t* decodedInstruction)
          : operandLocation(OperandLocationFromInstruction(decodedInstruction))
      {}

      /// Searches through the specified instruction for a position-dependent memory reference and
    ...
    /// Initializes the X86 instruction subsystem. Must be called once during program
    /// initialization.
    ...
    /// Determines if a jump instruction can be assembled from the specified location to the
    /// specified location.
    ...
    /// @return `true` if so, `false` if not or if either this instruction is invalid or no such
    /// memory reference exists.
    bool CanSetMemoryDisplacementTo(const int64_t displacement) const;

    /// Attempts to decode the instruction at the specified address, reading up to a maximum of the
    /// specified number of bytes. On success, this object captures the decoded representation of
    ...
    int EncodeInstruction(
        void* const buf, const int maxLengthBytes = kMaxInstructionLengthBytes) const;

    /// If this instruction contains a position-dependent memory reference, computes and returns the
    /// absolute target address of said reference.
    /// @return Absolute target address, or `nullptr` if either this instruction is invalid or no
    /// such memory reference exists.
    ...
    /// Retrieves and returns the number of bytes that represent the instruction in its encoded
    /// form.
    /// @return Number of bytes, or -1 if the instruction is invalid.
    int GetLengthBytes(void) const;

    /// If this instruction contains a position-dependent memory reference, computes and returns the
    /// maximum allowed displacement that can be incorporated into this instruction.
    /// @return Maximum displacement value, or #kInvalidMemoryDisplacement if either this
    /// instruction is invalid or no such memory reference exists.
    int64_t GetMaxMemoryDisplacement(void) const;

    /// If this instruction contains a position-dependent memory reference, determines the value of
    /// the displacement.
    /// @return Actual displacement value, or #kInvalidMemoryDisplacement if either this instruction
    /// is invalid or no such memory reference exists.
    int64_t GetMemoryDisplacement(void) const;

    /// If this instruction contains a position-dependent memory reference, determines the width in
    /// bits of the binary representation of the displacement value.
    /// @return Displacement value width in bits, or 0 if either this instruction is invalid or no
    /// such memory reference exists.
    int GetMemoryDisplacementWidthBits(void) const;

    /// If this instruction contains a position-dependent memory reference, computes and returns the
    /// minimum allowed displacement that can be incorporated into this instruction.
    /// @return Maximum displacement value, or #kInvalidMemoryDisplacement if either this
    /// instruction is invalid or no such memory reference exists.
    int64_t GetMinMemoryDisplacement(void) const;

    /// Determines if this instruction makes a memory reference whose address depends on this
    /// instruction's position. A memory reference for which this method returns `true` uses the
    /// value of the instruction pointer in computing the effective address. This could be because
    /// the instruction uses a relative branch displacement or, in 64-bit mode, RIP-relative
    /// addressing.
    /// @return `true` if so, `false` otherwise or if this instruction is invalid.
    inline bool HasPositionDependentMemoryReference(void)
    {
      return (
          PositionDependentMemoryReference::kReferenceDoesNotExist !=
          positionDependentMemoryReference.GetOperandLocation());
    }

    /// Determines if this instruction makes a memory reference, in the form of a relative branch
    /// displacement, whose target depends on this instruction's position. A memory reference for
    /// which this method returns `true` uses the value of the instruction pointer in computing the
    /// target of a possible jump. An instruction for which this method returns `false` but
    /// #HasPositionDependentMemoryReference returns `true` uses its position-dependent memory
    /// reference to access instruction operand data.
    /// @return `true` if so, `false` otherwise or if this instruction is invalid.
    inline bool HasRelativeBranchDisplacement(void)
    {
      return (
          PositionDependentMemoryReference::kReferenceIsRelativeBranchDisplacement ==
          positionDependentMemoryReference.GetOperandLocation());
    }

    /// Determines if this instruction is recognized as one that is often used for padding purposes
    /// and likely part of a padding buffer. Only a few single-byte instructions currently satisfy
    /// this criterion.
    /// @return `true` if so, `false` otherwise or if this instruction is invalid.
    bool IsPadding(void) const;

    /// If this instruction is one that is often used for padding purposes, check if this
    /// instruction is part of a padding buffer with at least the specified number of bytes after
    /// it. In order for this function to return `true` the this instruction must be recognized as a
    /// valid padding instruction and be followed by at least 3 identical instructions.
    /// @param [in] numBytes Number of bytes to check. Assumed to be the value 4 if it is less
    /// than 4.
    /// @return `true` if so, `false` otherwise or if this instruction is invalid.
    bool IsPaddingWithLengthAtLeast(int numBytes) const;

    /// Determines if this instruction marks the end of a control flow (i.e. end of a function,
    /// unconditional jump to someplace else, etc.)
    /// @return `true` if so, `false` otherwise or if this instruction is invalid.
    bool IsTerminal(void) const;

    /// Specifies whether the instruction represented by this object is valid.
    /// If not, no other methods to obtain information about the instruction will produce valid
    /// results.
    ...
    /// If this instruction contains a position-dependent memory reference, attempts to update the
    /// displacement value to the specified value.
  ...
  private:
    ...
    /// Holds information on any position-dependent memory accesses performed by this
    /// instruction, if any.
