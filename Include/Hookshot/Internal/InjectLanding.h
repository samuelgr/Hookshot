...
/// Entry point for the landing code. Written in assembly. Actually a jump target, not a function.
/// When this DLL has been loaded and initialized successfully, control transfers to this address.
/// At the time that this occurs, this DLL assumes control of the injection and hook setup process,
/// freeing the injecting process of all further responsibility. There is no return, but rather upon
/// completion control is transferred to the actual entry point of the injected process.
...
extern "C" void __fastcall InjectLandingLoadHookModules(
    const Hookshot::SInjectData* const injectData);
