...
namespace Hookshot
{
    ...
    /// Main interface used to access all Hookshot functionality.
    /// During initialization, Hookshot creates instances of objects that implement this interface as needed.
    /// Any hook modules that Hookshot loads are provided with an interface pointer when executing their entry point functions.
    /// Alternatively, if loading the Hookshot library directly, an interface pointer is returned upon completion of library initialization.
    /// Interface pointers remain valid throughout the lifetime of the process and are owned by Hookshot.
    /// Its methods can be called at any time and are completely concurrency-safe.
    /// However, it is highly recommended that results be cached where possible, because most methods require taking some form of a lock.
    class IHookshot
    {
    public:
