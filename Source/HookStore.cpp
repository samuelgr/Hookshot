...
namespace Hookshot
{
    ...
    static inline bool RedirectExecution(void* from, const void* to)
    {
        ...
        if (0 == Protected::Windows_VirtualProtect(from, X86Instruction::kJumpInstructionLengthBytes, PAGE_EXECUTE_READWRITE, &originalProtection))
            return false;
        ...
        const bool restoreProtectionResult = (0 != Protected::Windows_VirtualProtect(from, X86Instruction::kJumpInstructionLengthBytes, originalProtection, &unusedOriginalProtection));
        if (true == restoreProtectionResult)
            Protected::Windows_FlushInstructionCache(Globals::GetCurrentProcessHandle(), from, (SIZE_T)X86Instruction::kJumpInstructionLengthBytes);
