...
namespace Hookshot
{
    namespace ProcessInjector
    {
        ...
        {
            // This function attaches to the specified process as if debugging it and waits for it to trigger the breakpoint that indicates the initial load process has completed.
            // Windows will generate an exception breakpoint once the loader finishes initializing the process but before it runs.
            // At that point the process is still suspended. The main thread will not execute until `ResumeThread` is invoked on it.
        ...
        /// @param [in] processHandle Handle to the process that contains the module for which NT optional headers are desired.
        /// @param [in] baseAddress Base image address of the loaded module, in the address space of the specified process, for which NT optional headers are desired.
        /// @param [out] optionalHeader Buffer to be filled with the optional header data, if the operation is successful.
        /// @return Indicator of the result of the operation.
        static EInjectResult FillNtOptionalHeader(const HANDLE processHandle, const void* const baseAddress, IMAGE_OPTIONAL_HEADER* optionalHeader)
        {
            ...
            if ((FALSE == ReadProcessMemory(processHandle, (LPCVOID)((size_t)baseAddress + (size_t)ntHeadersOffset + (size_t)offsetof(IMAGE_NT_HEADERS, OptionalHeader)), (LPVOID)optionalHeader, sizeof(IMAGE_OPTIONAL_HEADER), (SIZE_T*)&numBytesRead)) || (sizeof(IMAGE_OPTIONAL_HEADER) != numBytesRead))
                return EInjectResult::ErrorReadNTHeadersFailed;

            return EInjectResult::Success;
        }

        /// Attempts to retrieve the handle for the `ntdll.dll` module which should be loaded in this process and all created processes, even if suspended.
        /// @return Handle of `ntdll.dll`, or `nullptr` in the event of a failure to locate it.
        static HMODULE GetNtDllModule(void)
        {
            static const HMODULE ntdllModuleHandle = LoadLibraryEx(L"ntdll.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
        ...
        /// @return Handle of the specified module in the specified process, or `nullptr` if the operation failed.
        static HMODULE GetRemoteModuleHandle(HANDLE processHandle, std::wstring_view moduleName)
        {
            ...
            if (FALSE == EnumProcessModules(processHandle, loadedModules.Data(), loadedModules.CapacityBytes(), &numLoadedModules))
                return nullptr;
            ...
            for (DWORD modidx = 0; modidx < numLoadedModules; ++modidx)
            {
                ...
                loadedModuleName.UnsafeSetSize(GetModuleFileNameEx(processHandle, loadedModule, loadedModuleName.Data(), loadedModuleName.Capacity()));
                if (true == loadedModuleName.Empty())
                    return nullptr;

                if (true == Strings::EndsWithCaseInsensitive<wchar_t>(loadedModuleName.AsStringView(), moduleName))
                    return loadedModule;
            }
        ...
        /// @param [in] moduleHandle Handle to the module, which must be loaded in the specified process, that is to be searched for an exported procedure.
        /// @param [in] procName Name of the exported procedure for which a pointer is requested.
        /// @return Handle of the specified module in the specified process, or `nullptr` if the operation failed.
        static FARPROC GetRemoteProcAddress(HANDLE processHandle, HMODULE moduleHandle, std::string_view procName)
        {
            ...
            do {
                ...
                EInjectResult operationResult = FillNtOptionalHeader(processHandle, (const void*)moduleHandle, &optionalHeader);
                if (EInjectResult::Success != operationResult)
                    return nullptr;

                // Index 0 in the data directory table contains export table information.
                // See https://learn.microsoft.com/en-us/windows/win32/debug/pe-format#optional-header-data-directories-image-only for more information.
                // Read the entire export address table, including all function names and pointers, into a vector.
                // Keeping the relative base address allows relative virtual addresses in the export table directory to be converted to vector byte indices.
                ...
                size_t numBytesRead = 0;
                if ((FALSE == ReadProcessMemory(processHandle, (LPCVOID)((size_t)moduleHandle + (size_t)moduleExportTableRelativeBaseAddress), (LPVOID)moduleExportTable.data(), (SIZE_T)moduleExportTable.size(), (SIZE_T*)&numBytesRead)) || ((size_t)moduleExportTable.size() != numBytesRead))
                    return nullptr;
            } while (false);

            // The entire export address table is stored in a vector, and the very beginning of it is a directory.
            // From the directory we obtain pointers to three arrays:
            // (1) An array of export function names (each element being a 4-byte relative virtual address of the beginning of the name string)
            // (2) An array of name ordinals (each element being a 2-byte index into the array below)
            // (3) An array of export function relative virtual addresses (each element being a 4-byte relative virtual address of the starting location of the function)
            const IMAGE_EXPORT_DIRECTORY* moduleExportDirectory = (const IMAGE_EXPORT_DIRECTORY*)moduleExportTable.data();
            const DWORD* exportFunctionNameArray = (DWORD*)((size_t)moduleExportTable.data() + (size_t)moduleExportDirectory->AddressOfNames - moduleExportTableRelativeBaseAddress);
            const WORD* exportFunctionNameOrdinalArray = (WORD*)((size_t)moduleExportTable.data() + (size_t)moduleExportDirectory->AddressOfNameOrdinals - moduleExportTableRelativeBaseAddress);
            const DWORD* exportFunctionAddressArray = (DWORD*)((size_t)moduleExportTable.data() + (size_t)moduleExportDirectory->AddressOfFunctions - moduleExportTableRelativeBaseAddress);

            // For each name, we need to compare it with the requested procedure name.
            // If it is a match, its index in the array of function names is used as an index into the ordinal array, which in turn gives an index into the function address array.
            std::optional<WORD> requestedProcOrdinal = std::nullopt;

            for (unsigned int procIndex = 0; (procIndex < moduleExportDirectory->NumberOfNames) && (false == requestedProcOrdinal.has_value()); ++procIndex)
            {
                std::string_view exportFunctionName = (const char*)((size_t)moduleExportTable.data() + (size_t)exportFunctionNameArray[procIndex] - moduleExportTableRelativeBaseAddress);
            ...
            return (FARPROC)((size_t)moduleHandle + (size_t)exportFunctionAddressArray[requestedProcOrdinal.value()]);
        }

        /// Attempts to determine the address of the entry point of the given process by locating the CLR entry point.
        /// This alternative mechanism is required for those processes that are managed and hence use a different loading process.
        /// In these special cases the entry point is actually the address of the `_CorExeMain` function exported by `mscoree.dll` rather than the entry point contained in the process' own header.
        /// See https://learn.microsoft.com/en-us/dotnet/framework/unmanaged-api/hosting/corexemain-function for more information.
        /// @param [in] processHandle Handle to the process for which information is requested.
        /// @param [in] processEnvironmentBlock Read-only reference to the process environment block which has already been read from the process of interest.
        /// @param [out] entryPoint Address of the pointer that receives the entry point address.
        /// @return Indicator of the result of the operation.
        static EInjectResult GetClrEntryPointAddress(const HANDLE processHandle, void** const entryPoint)
        {
            const HMODULE clrModuleHandle = GetRemoteModuleHandle(processHandle, L"mscoree.dll");
            if (nullptr == clrModuleHandle)
                return EInjectResult::ErrorGetModuleHandleClrLibraryFailed;

            void* clrEntryPoint = (void*)GetRemoteProcAddress(processHandle, clrModuleHandle, "_CorExeMain");
            if (nullptr == clrEntryPoint)
                return EInjectResult::ErrorGetProcAddressClrEntryPointFailed;
        ...
        {
            IMAGE_OPTIONAL_HEADER optionalHeader;
            EInjectResult operationResult = FillNtOptionalHeader(processHandle, baseAddress, &optionalHeader);
            if (EInjectResult::Success != operationResult)
                return operationResult;

            // Index 14 in the data directory table contains CLR metadata.
            // Its presence indicates that the executable is managed and therefore the load process is different.
            // See https://learn.microsoft.com/en-us/windows/win32/debug/pe-format#optional-header-data-directories-image-only for more information.
            if (0 != optionalHeader.DataDirectory[14].Size)
            {
                Message::Output(Message::ESeverity::Info, L"Process appears to be managed by the CLR. Using the CLR library's entry point address.");
                return GetClrEntryPointAddress(processHandle, entryPoint);
            }
            else
            {
                Message::Output(Message::ESeverity::Info, L"Process appears to be unmanaged by the CLR. Using the executable's own entry point address.");
                *entryPoint = (void*)((size_t)baseAddress + (size_t)optionalHeader.AddressOfEntryPoint);
                return EInjectResult::Success;
            }
        }
        ...
        /// @param [in] processHandle Handle to the process for which information is requested.
        /// @param [out] processEnvironmentBlock Pointer to a buffer to be filled with the contents of the process environment block from the requested process.
        /// @return Indicator of the result of the operation.
        static EInjectResult GetProcessEnvironmentBlock(const HANDLE processHandle, PEB* processEnvironmentBlock)
        {
            // This function uses the documented, but internal, Windows API function NtQueryInformationProcess.
            // See https://docs.microsoft.com/en-us/windows/win32/api/winternl/nf-winternl-ntqueryinformationprocess for official documentation.
            // While unlikely, if NtQueryInformationProcess is modified or made unavailable in a future version of Windows, this method might need to branch based on Windows version.

            static const NTSTATUS(WINAPI * ntdllQueryInformationProcessProc)(HANDLE, PROCESSINFOCLASS, PVOID, ULONG, PULONG) = ((nullptr == GetNtDllModule()) ? nullptr : (decltype(ntdllQueryInformationProcessProc))GetProcAddress(GetNtDllModule(), "NtQueryInformationProcess"));

            if (nullptr == GetNtDllModule())
            ...
            size_t numBytesRead = 0;
            if ((FALSE == ReadProcessMemory(processHandle, (LPCVOID)processBasicInfo.PebBaseAddress, (LPVOID)processEnvironmentBlock, sizeof(PEB), (SIZE_T*)&numBytesRead)) || (sizeof(PEB) != numBytesRead))
        ...
        /// @param [in] processEnvironmentBlock Read-only reference to the process environment block which has already been read from the process of interest.
        /// @param [out] baseAddress Address of the pointer that receives the image base address in the virtual address space of the given process.
        /// @return Indicator of the result of the operation.
        static EInjectResult GetProcessImageBaseAddress(const PEB& processEnvironmentBlock, void** const baseAddress)
        {
            ...
            *baseAddress = *((void**)((const uint8_t*)&processEnvironmentBlock + kByteOffsetPebImageBaseAddress));
            return EInjectResult::Success;
        }
