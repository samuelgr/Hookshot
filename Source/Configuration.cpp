...
namespace Configuration
{
    ...
    std::unique_ptr<Configuration> ConfigurationFileReaderBase::ReadConfigurationFile(const TCHAR* const configFileName)
    {
        FileHandle configFileHandle;
        _tfopen_s(&configFileHandle, configFileName, _T("r"));
        
        if (NULL == configFileHandle)
        {
            Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s - Unable to open configuration file."), configFileName);
            return NULL;
        }
        ...
        while (configLineLength >= 0)
        {
            ...
            case ELineClassification::Error:
                Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s:%d - Unable to parse line."), configFileName, configLineNumber);
                return NULL;
            ...
            case ELineClassification::Section:
                {
                    ...
                    {
                        Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s:%d - Section \"%s\" is duplicated."), configFileName, configLineNumber, section.c_str());
                        return NULL;
                    }
                    ...
                    case ESectionAction::Error:
                        Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s:%d - Section \"%s\" is invalid."), configFileName, configLineNumber, section.c_str());
                        return NULL;
                    ...
                    default:
                        Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s:%d - Internal error while processing section name."), configFileName, configLineNumber);
                        return NULL;
                    }
                }
                break;
            ...
            case ELineClassification::Value:
                if (false == skipValueLines)
                {
                    ...
                    {
                        Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s:%d - Value cannot appear outside of a section."), configFileName, configLineNumber);
                        return NULL;
                    }
                    ...
                    case EValueType::Integer:
                        {
                            ...
                            {
                                Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s:%d - Value \"%s\" is not a valid integer."), configFileName, configLineNumber, value.c_str());
                                return NULL;
                            }

                            if (false == CheckValue(thisSection, name, intValue))
                            {
                                Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s:%d - Configuration setting \"%s\" with value \"%s\" is invalid."), configFileName, configLineNumber, name.c_str(), value.c_str());
                                return NULL;
                            }
                    ...
                    case EValueType::Boolean:
                        {
                            ...
                            {
                                Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s:%d - Value \"%s\" is not a valid Boolean."), configFileName, configLineNumber, value.c_str());
                                return NULL;
                            }
                    ...
                    default:
                        Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s:%d - Internal error while processing configuration setting."), configFileName, configLineNumber);
                        return NULL;
                    }
                }
                break;
            ...
            default:
                Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s:%d - Internal error while processing line."), configFileName, configLineNumber);
                return NULL;
            }
        ...
        if (!feof(configFileHandle))
        {
            ...
            {
                Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s - I/O error while reading."), configFileName, configLineNumber);
                return NULL;

            }
            else if (configLineLength < 0)
            {
                Message::OutputFormatted(EMessageSeverity::MessageSeverityError, _T("%s:%d - Line is too long."), configFileName, configLineNumber);
                return NULL;
            }
        }
